...
define('BOT_TOKEN', '1086369008:AAEK15vAr50P1fGBo6X3HXgdPaMHYgCvrLU');
...
function channelReq($chat_id)
{
	$json = file_get_contents('https://api.telegram.org/bot' . BOT_TOKEN . "/getChatMember?chat_id=-1001313653081&user_id=" . $chat_id);
	$data = json_decode($json, true);
	return $data["result"]["status"];
}
// $chat_id = $message['chat']['id'];
$json = file_get_contents('https://api.telegram.org/bot1086369008:AAEK15vAr50P1fGBo6X3HXgdPaMHYgCvrLU/getChatMember?chat_id=-1001313653081&user_id=' . $chat_id);
$truechannel = json_decode($json, true);
...
function processMessage($message)
{
	...
	if (isset($message['text'])) {
		...
		if (strpos($text, "/start") === 0) {
			(string) $tsh = channelReq($chat_id);
			if ($tsh == "member" || $tsh == "creator" || $tsh == "administrator") {
				apiRequestJson("sendMessage", array('chat_id' => $chat_id, "text" => "سلام 🤖 \nاگر تو هم تلگرامت آپدیت باشه حتما این دکمه های شیشه ای رو دیدی پس میریم سر اصل مطلب👏\nبرای شروع بازی XO روی دکمه شروع بازی کلیک کن\nبعد هم یکی از دوستاتو انتخاب کن و منتظر بمون تا باکس inline برای تو باز بشه و بعد روی شروع بازی XO رو بزن\nبه همین راحتی 🤙", 'reply_markup' => array(
					"inline_keyboard" => array(
						array(array("text" => "👨‍💻 توسعه دهنده 👨‍💻", "url" => "https://telegram.me/IEfiwm")),
						array(array("text" => "👨‍💻 پشتیبانی 👨‍💻", "url" => "https://telegram.me/IEfiwmPVbot")),
						array(array("text" => "📣 کانال ما 📣", "url" => "https://telegram.me/Team_Freenet_Official")),
						array(array("text" => "➕ شروع بازی ➕", "switch_inline_query" => md5(date("YMDms"))))
					)
				)));
			} else {
				apiRequestJson("sendMessage", array('chat_id' => $chat_id, "text" => "سلام 🤖 \nمتاسفانه عضو کانال ما نیستی 😐\n اگر میخوای بتونی از من استفاده کنی عضو کانال ما شو\nو بعدش /start رو بزن\n", 'reply_markup' => array(
					"inline_keyboard" => array(
						array(array("text" => " 👨‍💻 توسعه دهنده 👨‍💻", "url" => "https://telegram.me/IEfiwm")),
						array(array("text" => "👨‍💻 پشتیبانی 👨‍💻", "url" => "https://telegram.me/IEfiwmPVbot")),
						array(array("text" => "📣 کانال ما 📣", "url" => "https://telegram.me/Team_Freenet_Official"))
...
function inlineMessage($inline)
{
	...
	apiRequest("answerInlineQuery", array("inline_query_id" => $id, 
	"results" => array(array(
		"type" => "article", 
		"id" => $query, 
		"title" => "شروع بازی XO", 
		"input_message_content" => array("message_text" => "<b>بازی XO</b>\n برای شروع روی دگمه زیر کلیک کنید👇🏻👇🏻👇🏻", 
		"parse_mode" => "HTML", 
